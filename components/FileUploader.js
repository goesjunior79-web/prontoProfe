  ...
  return (
    <div style={{ background: '#fff', border: '0.5px solid #E0DDD5', borderRadius: 12, padding: '1rem', marginBottom: 12 }}>
      ...
      >
        ...
        <div style={{ fontSize: 11, color: '#aaa', marginTop: 6 }}>PDF · Word · Imagem · TXT</div>
      </div>
              ...
              <div key={idx} style={{ background: '#F7F6F3', border: '0.5px solid #E0DDD5', borderRadius: 9, padding: '9px 11px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: '#aaa' }}>{fmtSize(f.size)}</div>
                  </div>
                  {f.status === 'ok' && <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 20, background: '#EAF3DE', color: '#3B6D11' }}>{f.note || (f.text ? `~${Math.round(f.text.length / 1000)}k` : 'img')}</span>}
                  {(f.status === 'reading' || f.status === 'extracting') && <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 20, background: '#FAEEDA', color: '#633806' }}>{f.status === 'reading' ? 'Lendo...' : 'Extraindo...'}</span>}
                ...
                {f.status === 'range' && (
                  <div style={{ marginTop: 9, background: '#FAEEDA', borderRadius: 7, padding: '9px 11px' }}>
                    <div style={{ fontSize: 11, fontWeight: 500, color: '#633806', marginBottom: 7 }}>{f.totalPages} páginas — intervalo:</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
                      ...
                      <button style={btnPri} onClick={() => onExtractPages(idx)}>Extrair</button>
                    </div>
                  </div>
                )}
              </div>
