...
const COLUMNS = 'id, user_id, nome, descricao, tipo, storage_path, size_bytes, is_default, created_at, deleted_at';

export async function createModelo({ userId, nome, descricao = null, tipo, storagePath, sizeBytes, isDefault = false }) {
  if (!userId || !nome || !tipo || !storagePath || sizeBytes == null) {
    throw new Error('createModelo: parâmetros obrigatórios faltando');
  }
  // Se isDefault=true, primeiro desmarca os outros (1 default por user)
  if (isDefault) {
