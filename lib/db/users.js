 ...
 *
 * @param {string} userId
 * @param {string} [userEmail] — passado para limpar tabelas legacy (profiles/generations)
 */
export async function deleteAccount(userId, userEmail = null) {
  ...
  // Tabelas legacy (Fase 10) — não têm user_id, identificadas por email.
  // Não têm deleted_at column; faz hard delete (são caches do MVP antigo).
  if (userEmail) {
    await supabase.from('profiles').delete().eq('email', userEmail);
    await supabase.from('generations').delete().eq('email', userEmail);
  }
